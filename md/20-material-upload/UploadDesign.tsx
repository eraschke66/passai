...
export const MaterialsPage = ({ preSelectedSubjectId }: MaterialsPageProps) => {
  ...
  const [selectedSubject, setSelectedSubject] = useState<Subject | null>(
    preSelectedSubjectId
      ? mockSubjects.find((s) => s.id === preSelectedSubjectId) || null
      : null
  );
  ...
  const filteredMaterials = materials.filter((material) => {
    const matchesSubject =
      !filterSubject || material.subject === filterSubject.name;
  ...
  const handleFileSelect = (files: FileList | null) => {
    ...
    if (totalFiles > UPLOAD_LIMITS.maxFiles) {
      alert(
        `You can only upload up to ${UPLOAD_LIMITS.maxFiles} files at once. You currently have ${pendingFiles.length} pending.`
      );
      return;
    }
    ...
    fileArray.forEach((file) => {
      ...
      if (sizeMB > UPLOAD_LIMITS.maxSizePerFile) {
        errors.push(
          `${file.name}: File size exceeds ${UPLOAD_LIMITS.maxSizePerFile}MB`
        );
        return;
      }
    ...
    // Check total size
    const existingSize = pendingFiles.reduce(
      (acc, pf) => acc + pf.file.size / (1024 * 1024),
      0
    );
    const newSize = validFiles.reduce(
          ...
          if (currentStage >= stages.length) {
            ...
            setTimeout(() => {
              const newMaterial: Material = {
                ...
                type: pendingFile.file.type.includes("pdf")
                  ? "pdf"
                  : pendingFile.file.type.includes("video")
                  ? "video"
                  : pendingFile.file.type.includes("image")
                  ? "image"
                  : "document",
                subject: pendingFile.subject,
                subjectColor: pendingFile.subjectColor,
                uploadedDate: "Just now",
                size: `${(pendingFile.file.size / (1024 * 1024)).toFixed(
                  1
                )} MB`,
                status: "ready",
              };
  ...
  return (
    <div className="h-full overflow-y-auto pb-4">
            ...
            <div className="hidden lg:block relative">
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all ${
                  filterSubject
                    ? `bg-gradient-to-r ${filterSubject.color} text-white shadow-md hover:shadow-lg`
                    : "bg-white border-slate-200 text-slate-700 hover:border-slate-300"
                }`}
              >
                <Book className="w-5 h-5" />
                <span className="font-semibold">
                  {filterSubject?.name || "All Subjects"}
                </span>
                <ChevronDown
              ...
              {isSubjectDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-xl border-2 border-slate-200 shadow-xl z-10">
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors first:rounded-t-xl ${
                      !filterSubject ? "bg-blue-50" : ""
                    }`}
                  >
                  ...
                  {mockSubjects.map((subject) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors last:rounded-b-xl ${
                        filterSubject?.id === subject.id ? "bg-blue-50" : ""
                      }`}
                    >
                      <div
            ...
            <div className="lg:hidden -mx-4 px-4 overflow-x-auto hide-scrollbar">
              <div className="flex gap-2 pb-1">
                <button
                  ...
                  className={`flex-shrink-0 px-3 py-2 rounded-xl transition-all active:scale-95 ${
                    !filterSubject
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-white rounded-xl p-3 lg:p-4 border-2 border-slate-200">
              ...
              <p className="text-xl lg:text-3xl font-bold text-blue-600">
                {stats.totalSize.toFixed(0)}MB
              </p>
            </div>
            <div
              className={`rounded-xl p-3 lg:p-4 text-white ${
                isProcessing
            ...
            >
              ...
              <p className="text-xl lg:text-3xl font-bold">
                {isProcessing ? stats.processing : stats.pending}
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-amber-800">
                    Please do not close or exit the app until all materials are
                ...
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 mb-4">
                      Upload any study materials to help PassAI create
                    ...
                    <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-amber-800">
                          Upload the teacher's emphasis! If your teacher said
                          "This will definitely be on the test," add a note
                          about it.
                        </p>
                      </div>
                    </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-xl hover:shadow-lg active:scale-95 transition-all"
                >
              ...
              <div className="space-y-2">
                {pendingFiles.map((pending) => {
                  ...
                  return (
                    <div
                      key={pending.id}
                      className="bg-white border-2 border-blue-200 rounded-xl p-3 lg:p-4 hover:border-blue-300 transition-all group"
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 lg:w-12 lg:h-12 rounded-lg bg-gradient-to-br ${getFileColor(
                            pending.file
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2">
                            <div
                              className={`px-2 py-0.5 rounded text-xs font-semibold bg-gradient-to-r ${pending.subjectColor} text-white`}
                            >
                              {pending.subject}
                            </div>
                            <span className="text-xs lg:text-sm text-slate-600">
                              {(pending.file.size / (1024 * 1024)).toFixed(1)}{" "}
                              MB
                            </span>
                          </div>
                        </div>
                        <button
                          ...
                          className="w-8 h-8 lg:w-9 lg:h-9 rounded-lg bg-red-50 hover:bg-red-100 flex items-center justify-center transition-colors flex-shrink-0"
                          title="Remove file"
                        >
              ...
              <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <p className="text-sm text-blue-800">
                  Review your files and click{" "}
                  <span className="font-bold">"Process All"</span> to start
                  uploading. You can add more files or remove any before
                  processing.
                </p>
              </div>
            </div>
          ...
          {processingFiles.length > 0 && (
            <div className="space-y-3">
              ...
              {processingFiles.map((file) => (
                <div
                  key={file.id}
                  className="bg-white border-2 border-blue-200 rounded-xl p-4 lg:p-5"
                >
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <div
                          className={`px-2 py-0.5 rounded text-xs font-semibold bg-gradient-to-r ${file.subjectColor} text-white`}
                        >
                          {file.subject}
                        </div>
                ...
                {filteredMaterials.map((material) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3 mb-3">
                        <div
                          className={`w-12 h-12 rounded-xl bg-gradient-to-br ${getMaterialColor(
                            material.type
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <div
                              className={`px-2 py-0.5 rounded text-xs font-semibold bg-gradient-to-r ${material.subjectColor} text-white`}
                            >
                              {material.subject}
                            </div>
                            <span className="text-xs text-slate-600">
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <p className="text-xs text-slate-600">
                        Upload materials for this subject
                      </p>
                    </div>
      ...
      {showUploadZone && uploadSubject && (
        <div
          className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300"
          onClick={() => !isProcessing && setShowUploadZone(false)}
        >
          <div
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm text-blue-800">
                    ...
                    <ul className="space-y-0.5 text-xs">
                      <li>
                        • Maximum {UPLOAD_LIMITS.maxFiles} files per upload
                      </li>
                      <li>
                        • Maximum {UPLOAD_LIMITS.maxSizePerFile}MB per file
                      </li>
              ...
              >
                ...
                <div
                  className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${
                    isProcessing ? "bg-slate-100" : "bg-blue-100"
                  }`}
                >
                  {isProcessing ? (
                ...
                <p className="text-sm text-slate-600 mb-4">
                  {isProcessing
                    ? "Please wait while we process your materials"
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {pendingFiles.map((pending) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-8 h-8 rounded-lg bg-gradient-to-br ${getFileColor(
                              pending.file
